...
### Precipitation Regressions                               ###
###############################################################
# Created by Fisher Ankney, January 2017 - fisherankney@gmail.com

## Import the weather station data 
  dependent_prec_dataframe <- as.data.frame() # <-- Enter dependent climate data, 13 column format
  first_prec_dataframe <- as.data.frame() # <-- Enter independent climate data, 13 column format
  second_prec_dataframe <- as.data.frame() # <-- Enter independent climate data, 13 column format
       
## Adjust for the years you want to calculate  
                                                              ####### Adjust these numbers to fit data  
  dependent_prec_dataframe <- (dependent_prec_dataframe[c(seq(1,200,by=1)),])
  names(dependent_prec_dataframe) <- c("year", "dep_jan", "dep_feb", "dep_mar", 
                                  "dep_apr", "dep_may", "dep_jun", "dep_jul",
                                  "dep_aug", "dep_sep", "dep_oct", "dep_nov",
                                  "dep_dec")  
                                                      ###### Adjust these numbers to fit data
  first_prec_dataframe <- (first_prec_dataframe[c(seq(1,200,by=1)),])
  names(first_prec_dataframe) <- c("year", "ind_jan", "ind_feb", "ind_mar", 
                                  "ind_apr", "ind_may", "ind_jun", "ind_jul",
                                  "ind_aug", "ind_sep", "ind_oct", "ind_nov",
                                  "ind_dec") 
                                                        ####### Adjust these numbers to fit data
  second_prec_dataframe <- (second_prec_dataframe[c(seq(1,200,by=1)),])
  names(second_prec_dataframe) <- c("year", "ind2_jan", "ind2_feb", "ind2_mar", 
...
## Merge these two dataframes
  partial_dataframe <- merge(dependent_prec_dataframe, first_prec_dataframe)
  combined_dataframe <- merge(partial_dataframe, second_prec_dataframe)
